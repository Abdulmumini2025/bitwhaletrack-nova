...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const getEmailSubject = (actionType: string) => {
      switch (actionType) {
        case 'signup':
          return '🚀 Welcome to Bitwhaletrack - Verify Your Account'
        case 'recovery':
          return '🔒 Reset Your Bitwhaletrack Password'
        case 'email_change':
          return '📧 Confirm Your New Email Address'
        default:
          return '🔐 Bitwhaletrack Account Verification'
      }
    }

    // Send email using Resend
    const { error } = await resend.emails.send({
      from: 'Bitwhaletrack <auth@bitwhaletrack.com>',
      to: [user.email],
