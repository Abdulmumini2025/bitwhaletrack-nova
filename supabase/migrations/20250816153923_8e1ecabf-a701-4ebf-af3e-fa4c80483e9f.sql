-- Create remaining types if they don't exist
DO $$ BEGIN
...
-- News policies
DROP POLICY IF EXISTS "Anyone can view approved news" ON public.news;
...
DROP POLICY IF EXISTS "Users can update their own pending news" ON public.news;
DROP POLICY IF EXISTS "Admins can view all news" ON public.news;
DROP POLICY IF EXISTS "Admins can update any news" ON public.news;
...
CREATE INDEX IF NOT EXISTS idx_likes_user_news ON public.likes(user_id, news_id);
